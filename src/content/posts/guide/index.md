...
这个博客模板是使用 [Astro](https://astro.build/) 构建的。本指南中未提及的内容，您可以在 [Astro 文档](https://docs.astro.build/) 中找到答案。
...
| `image`       | 文章的封面图片路径。<br/>1. 以 `http://` 或 `https://` 开头：使用网络图片<br/>2. 以 `/` 开头：使用 `public` 目录中的图片<br/>3. 不带前缀：相对于 Markdown 文件的路径                                |
| `tags`        | 文章的标签。                                                                                                                                                                                         |
| `category`    | 文章的分类。                                                                                                                                                                                         |
...
您的文章文件应该放在 `src/content/posts/` 目录中。您也可以创建子目录来更好地组织您的文章和资源。
